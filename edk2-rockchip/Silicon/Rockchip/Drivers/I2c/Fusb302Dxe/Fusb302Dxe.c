  ...
  )
{
  ...
  for (Count = 0; Count < FUSB302_MAX_PDOS; Count++) {
    if (Pdos[Count] == 0) {
      break;
    }
  }
...
//
// An I2C request packet ends in a flexible array of operations, so a packet
// holding a given number of them needs a type that carries them with it. A
// register access is over by the time the call returns, so these live on the
// stack: the PD state machine reads a register every 500 us, and there is no
// reason for that to churn the allocator.
//
typedef struct {
  ...
  )
{
  ...
  Status = Context->I2cIo->QueueRequest (
                             Context->I2cIo,
                             0,
                             NULL,
  ...
  )
{
  ...
  //
  // Our own Rp holds an unconnected pin at the top of the range, the partner's
  // Rd pulls the pin it is on below that, and an electronically marked cable's
  // Ra pulls the VCONN pin lower still. So a pin at the top is simply open and
  // the partner is on the other one; where neither is at the top both are
  // loaded, and the higher of the two is Rd rather than Ra.
  //
  if (Cc1Level == FUSB302_BC_LVL_3A) {
    *Orientation = UsbTypeCOrientationFlipped;
  } else if (Cc2Level == FUSB302_BC_LVL_3A) {
    *Orientation = UsbTypeCOrientationNormal;
  } else {
    *Orientation = (Cc1Level > Cc2Level) ? UsbTypeCOrientationNormal
                                         : UsbTypeCOrientationFlipped;
  }
...
Restore:
  //
  // Leave the part presenting a sink termination either way. Sourcing puts the
  // Rp back itself, alongside the supply it is advertising; holding one here
  // would offer a current this board has not yet decided to deliver.
  //
  ...
  )
{
  ...
  //
  // A bus with nothing driving it reads back all zeroes or all ones. Check
  // before the reset below, which would otherwise go to whatever does sit at
  // this address. Revisions this driver has not been tried against are let
  // through: the identifier is only being used to tell a part from an empty
  // bus.
  //
  if ((DeviceId == 0x00) || (DeviceId == 0xFF)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: no FUSB302 answering here (device ID 0x%02x)\n",
      __func__,
      DeviceId
      ));
    return EFI_UNSUPPORTED;
...
/**
  Find the board's description of the port this I2C device is, if it named one.

  @param[in]  I2cIo   The device being offered to the driver.

  @retval NULL   No description names this device.
  ...
  )
{
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    if ((I2cIo->DeviceIndex == Platform->DeviceIndex) &&
        CompareGuid (I2cIo->DeviceGuid, &Platform->DeviceGuid))
    {
      FreePool (Handles);
      return Platform;
    }
  ...
  )
{
  ...
  if (Fusb302MatchPlatformPort (TmpI2cIo) != NULL) {
    Status = EFI_SUCCESS;
  } else {
  ...
  )
{
  ...
  //
  // Supported() only let us this far for a device the board described, so
  // the same lookup cannot come back empty here.
  //
  Context->Platform = Fusb302MatchPlatformPort (I2cIo);
  ASSERT (Context->Platform != NULL);
  if (Context->Platform == NULL) {
    Status = EFI_NOT_FOUND;
    goto FreeContext;
  }

  Context->TypeCPort.PhyId = Context->Platform->PhyId;
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    // Only worth asking for a contract where the partner supplies one. The
    // other direction is the sourcing path below.
    //
  ...
  )
{
  ...
  Context = FUSB302_SC_FROM_TYPEC_PORT (TypeCPort);

  //
  // Take the port away before the context behind it goes, so that nothing is
  // left holding a protocol whose I2C handle we are about to close.
  //
