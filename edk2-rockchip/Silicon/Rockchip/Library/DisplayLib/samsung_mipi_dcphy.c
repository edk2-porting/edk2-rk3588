...
#define SAMSUNG_MIPI_DCPHY_SIGNATURE          SIGNATURE_32 ('S', 'M', 'D', 'c')

#define SAMSUNG_MIPI_DCPHY_FROM_PHY(a) \
	CR (a, struct samsung_mipi_dcphy, phy, SAMSUNG_MIPI_DCPHY_SIGNATURE)
...
{
	struct samsung_mipi_dcphy *samsung = SAMSUNG_MIPI_DCPHY_FROM_PHY(phy);
...
struct samsung_mipi_dcphy mMipiDcPhy[] = {
	{
		.Signature = SAMSUNG_MIPI_DCPHY_SIGNATURE,
		.base = 0xfeda0000,
		.grf = 0xfd5e8000,
		.id = 0,
		.lanes = 4,
		.m_phy_rst	= { 0x30A10, 3 }, /* mresetn_mipi_dcphy0 */
		.phy = {
	...
	},
	{
		...
		.id = 1,
		.lanes = 4,
		.m_phy_rst	= { 0x30A10, 5 }, /* mresetn_mipi_dcphy1 */
		.phy = {
			.funcs = &samsung_mipi_dcphy_funcs
		},
	},
...
{
	for (int i = 0; i < ARRAY_SIZE(mMipiDcPhy); i++) {
		if (id == mMipiDcPhy[i].id) {
			return &mMipiDcPhy[i].phy;
		}
	}
	return NULL;
}
