  ...
  )
{
  ...
  //
  // Report what we can actually deliver. RngDxe registers each source it can
  // serve under its own algorithm, so this block gets a GUID of its own rather
  // than gEfiRngAlgorithmRaw -- that is already taken by the SMCCC TRNG, and
  // reusing it would list the same algorithm twice. Without the block we are no
  // better than BaseRngLibTimerLib and must say so, or callers will trust
  // numbers that do not deserve it.
  //
  if (ProbeTrng ()) {
    CopyGuid (RngGuid, &gRk3588RngAlgorithmTrngV1Guid);
  } else {
