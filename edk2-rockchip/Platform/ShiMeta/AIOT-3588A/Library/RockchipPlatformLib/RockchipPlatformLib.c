...
{
  DEBUG ((DEBUG_INFO, "Usb2PhyResume called\n"));

  // Release USB PHY resets
  ...
  // Configure PHY clocks in GRF
  MmioWrite32 (0xFD7F0A10, 0x07000700);
  MmioWrite32 (0xFD7F0A10, 0x07000000);

  // Enable USB power GPIOs
  GpioPinSetDirection (4, GPIO_PIN_PB5, GPIO_PIN_OUTPUT); // USB_HOST_PWREN
  GpioPinWrite (4, GPIO_PIN_PB5, TRUE);

  GpioPinSetDirection (1, GPIO_PIN_PD2, GPIO_PIN_OUTPUT); // TYPEC5V_PWREN
  GpioPinWrite (1, GPIO_PIN_PD2, TRUE);
}
  ...
  )
{
  switch (Id) {
    case 1:
      DEBUG ((DEBUG_INFO, "HdmiTxIomux init\n"));
      GpioPinSetFunction (2, GPIO_PIN_PC4, 4); // hdmim1_tx1_cec
      GpioPinSetPull (2, GPIO_PIN_PC4, GPIO_PIN_PULL_NONE);
      GpioPinSetFunction (3, GPIO_PIN_PC7, 5); // hdmim1_tx1_hpd
      GpioPinSetPull (3, GPIO_PIN_PC7, GPIO_PIN_PULL_NONE);
