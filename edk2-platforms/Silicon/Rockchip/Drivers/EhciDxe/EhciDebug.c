...
/**
  Dump the EHCI Ccontroller Registers.
...
{
  UINT32                  Data;
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + 0x00);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + 0x00, Data));
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + 0x04);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + 0x04, Data));
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + 0x08);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + 0x08, Data));
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + 0x0c);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + 0x0c, Data));
  //Read Operational Reg
  for(int i=0;i < 7; i++){
    Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + Ehc->CapLen + 0x04*i);
    DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + Ehc->CapLen +0x04*i, Data));
  }
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + Ehc->CapLen + 0x40);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + Ehc->CapLen +0x40, Data));
  Data = MmioRead32 (Ehc->UsbHostControllerBaseAddress + Ehc->CapLen + 0x44);
  DEBUG ((EFI_D_ERROR, "EhcDumpReg 0x%x = 0x%x\n",Ehc->UsbHostControllerBaseAddress + Ehc->CapLen +0x44, Data));
}