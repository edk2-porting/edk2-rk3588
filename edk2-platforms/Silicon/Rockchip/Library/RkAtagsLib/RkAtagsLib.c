  ...
  )
{
  ...
  if (TagHeader->Type != RkAtagTypeCore) {
    DEBUG ((DEBUG_ERROR, "%a: ATAGS not initialized!\n",
            __FUNCTION__));
    return NULL;
  }

  if (Type <= RkAtagTypeCore || Type > RkAtagTypeMax) {
    DEBUG ((DEBUG_ERROR, "%a: Type=%x unsupported!\n",
            __FUNCTION__, Type));
    return NULL;
  }
  ...
  while (TagHeader->Size > 0) {
    if ((UINTN)TagBuffer + ATAG_SIZE_BYTES(TagHeader->Size) - ATAGS_BASE > ATAGS_SIZE) {
      DEBUG ((DEBUG_ERROR, "%a: Type=%x overflown!\n",
    ...
    if (TagHeader->Type == Type) {
      ...
      if (ComputedTagHash != *TagHash) {
        DEBUG ((DEBUG_ERROR, "%a: Corrupted tag hash! Type=%x, TagHash=%x, ComputedTagHash=%x\n",
                __FUNCTION__, TagHeader->Type, *TagHash, ComputedTagHash));
        return NULL;
      }
  ...
  DEBUG ((DEBUG_ERROR, "%a: Tag not found! Type=%x.\n",
          __FUNCTION__, Type));
  return NULL;
}
